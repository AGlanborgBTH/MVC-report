...
class Poker
{
     ...
     */
    public function recieveBet(int $bet): void
    {
        $this->bet = $bet;
        $this->phase = 1;
    }
    ...
    /**
     * Method for updating the $this->bet property to twice the amount and dealing App\Poker\PokerCard
     ...
     */
    public function rankPlayer(): void
    {
     ...
     */
    public function rankDealer(): void
    {
     ...
     */
    public function compareRanks(): void
    {
        if ($this->player->hand_rank < $this->dealer->hand_rank) {
            $this->phase = 4;
        } elseif ($this->player->hand_rank == $this->dealer->hand_rank) {
            $tie = new PokerTie();
            $this->phase = $tie->result(
                $this->player_hand_rank,
                $this->dealer_hand_rank,
                $this->player->hand,
                $this->dealer->hand,
                $this->table->hand
            );
